...
from .transform import tabla_desde_log
...
class ResultadoFecha:
    ...
    Campos:
        ...
        operaciones (int): operaciones `resetuser` encontradas en el log.
...
class ResultadoEjecucion:
    ...
    def total_operaciones(self) -> int:
        """Operaciones `resetuser` leídas en todos los días procesados.
...
def preparar_staging(fecha: str, strict: bool = False) -> pd.DataFrame:
    ...
    staging = tabla_desde_log(path)
    revisar(validar_staging(staging, fecha), strict)
    return staging


def procesar_fecha(fecha: str, strict: bool = False) -> ResultadoFecha:
    ...
    """
    staging = preparar_staging(fecha, strict)
    destino = guardar_staging(staging, fecha)
    _, nuevos = upsert_reporte(staging)
    logger.info(
        "%s: %d operaciones resetuser, %d registros nuevos", fecha, len(staging), nuevos
    )
    return ResultadoFecha(fecha, len(staging), nuevos, destino)


def simular_fecha(
    fecha: str, ids_conocidos: set[str], strict: bool = False
) -> ResultadoFecha:
    ...
    logger.info(
        "%s: %d operaciones resetuser, %d registros nuevos (simulado)",
        fecha,
        len(staging),
        len(nuevos),
    )
...
def ejecutar(
    ...
    resultado = ResultadoEjecucion(dry_run=dry_run)
    if dry_run:
        ids_conocidos = set(cargar_reporte()["id"])
        base = len(ids_conocidos)
        resultado.fechas = [simular_fecha(f, ids_conocidos, strict) for f in fechas]
        resultado.total_reporte = base + resultado.total_nuevos
    else:
        resultado.fechas = [procesar_fecha(f, strict) for f in fechas]
